...
Now, take a pen and a scratch paper and try to understand the algorithm by solving the following example. Assume the function $h(s)$ gives the number of nodes $s$ is away from $s_{goal}$ and $g(s) = g(s_{start}) + c(s_{start},s)$, where $c$ is the edge cost. So, $h(s_{goal}) = 0$ and $g(s_{start}) = 0$.      
...
Node To Expand $min(f)$ | OPEN | CLOSED | bp(s)
--- | --- | ---
$s_{start}$ | {$s_{start}$} | {} | \-
...
Finally, in order to find the path we have to just follow the backpointers all the way from $s_{goal}$ to $s_{start}$.
