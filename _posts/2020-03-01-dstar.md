...
There's a simple solution to this: update the $g-values$ using new sensory information. The robot finds a solution, executes it, and finds that the environment has changed; updates $g-values$ according to the environment and sets current node to $s_{start}$.  

![eg]({{ site.url }}/assets/images/dstar.PNG)
