...
* All zero-valued pixels or free space neighbouring the goal are labeled with **three**.
...
![wave]({{ site.url }}/assets/gifs/wavefront.gif) 
<p align="center">
Figure 5.4 Wave-front Planner
</p>

The wave-front planner essentially forms a potential function on the grid which has one local minimum and thus is **resoultion complete**. The planner also determines the shortest path, but at the cost of coming dangerously close to the obstacles. Finally, the wave-front planner generalizes into higher dimensions as well. For a three-dimensional space, there will be a three-dimensional pixel or voxel(with six faces).  
